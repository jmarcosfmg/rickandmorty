...
public class UpdateLocationImpl extends LogUtils implements UpdateLocation {
    ...
    @Override
    public List<UpdateLocationOutput> execute(List<UpdateLocationInput> input) {
        if (input.isEmpty()) {
            log.warn("No locations to update");
            return List.of();
        }

        List<Integer> locationIds = input.parallelStream().map(UpdateLocationInput::id).toList();
        ...
        List<Location> updatedLocations = input.parallelStream()
                .map(l -> fetchedLocations.get(l.id())
                        .update(new Location(
                                l.name(),
                                l.dimension(),
                                getResidents(l.residents()))
                        )
                ).toList();
    ...
    private List<Character> getResidents(List<Integer> residents) {
        if (residents.isEmpty()) return null;
        return characterRepository.getCharacters(residents);
    }
